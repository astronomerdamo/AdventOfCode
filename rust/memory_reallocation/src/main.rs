...
fn find_max_bank_by_index(mem_banks: &Vec<Bank>) -> (u8, u16) {
    let max_bank = mem_banks.iter().cloned().fold(Bank {index: 0, blocks: 0}, |mut acc, s| {
...
fn main() {
    ...
    let (loop_count, loop_incidence) = loop {
        ...
        let matched_memory_state = match memory_states.iter().cloned().filter(|x| x.1 == memory).next() {
    ...
    println!("Reallocation moves between loop boundries {:?}", &loop_count - &loop_incidence);
}
